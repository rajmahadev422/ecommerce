...
import { User, Mail, Phone, MapPin, Package, Edit3, Save, X, ChevronRight, Calendar, ShoppingBag, LogOut, Loader2 } from 'lucide-react'
...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 page-enter">
      <h1 className="font-display text-3xl font-bold text-stone-900 dark:text-white mb-8">My Profile</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 space-y-5">
          ...
          <div className="card p-6 text-center">
            ...
            <h2 className="font-display text-xl font-bold text-stone-900 dark:text-white">{user.name}</h2>
            <p className="text-stone-500 dark:text-zinc-400 text-sm mt-1">{user.email}</p>
          ...
          <div className="card p-5 grid grid-cols-2 gap-4">
            <div className="text-center">
              <p className="text-2xl font-bold font-display text-stone-900 dark:text-white">{orders.length}</p>
              <p className="text-xs text-stone-400 dark:text-zinc-500 mt-0.5">Orders</p>
            </div>
            <div className="text-center border-l border-stone-100 dark:border-zinc-800">
              <p className="text-2xl font-bold font-display text-stone-900 dark:text-white">${totalSpent.toFixed(0)}</p>
              <p className="text-xs text-stone-400 dark:text-zinc-500 mt-0.5">Total Spent</p>
            </div>
          </div>

          {/* Quick links */}
          <div className="card overflow-hidden">
            <Link href="/orders" className="flex items-center justify-between px-5 py-3.5 hover:bg-stone-50 dark:hover:bg-zinc-800 transition-colors border-b border-stone-50 dark:border-zinc-800">
            ...
            <Link href="/" className="flex items-center justify-between px-5 py-3.5 hover:bg-stone-50 dark:hover:bg-zinc-800 transition-colors border-b border-stone-50 dark:border-zinc-800">
              <div className="flex items-center gap-3 text-sm font-medium text-stone-700 dark:text-zinc-300">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="card p-6">
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-display text-xl font-bold text-stone-900 dark:text-white">Personal Info</h3>
              {editing ? (
            ...
            {editing ? (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-stone-500 dark:text-zinc-400 mb-1.5">Full Name</label>
                    <input value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="input-field" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-stone-500 dark:text-zinc-400 mb-1.5">Phone</label>
                    <input value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} className="input-field" placeholder="+1 (555) 000-0000" />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-medium text-stone-500 dark:text-zinc-400 mb-1.5">Street Address</label>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-stone-500 dark:text-zinc-400 mb-1.5">City</label>
                    <input value={form.city} onChange={e => setForm({...form, city: e.target.value})} className="input-field" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-stone-500 dark:text-zinc-400 mb-1.5">State</label>
                    <input value={form.state} onChange={e => setForm({...form, state: e.target.value})} className="input-field" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-stone-500 dark:text-zinc-400 mb-1.5">ZIP</label>
                    <input value={form.zip} onChange={e => setForm({...form, zip: e.target.value})} className="input-field" />
                  </div>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                {[
                  ...
                  { icon: <Phone className="w-4 h-4" />, label: 'Phone', value: user.phone || 'Not set' },
                  {
                  ...
                  <div key={field.label} className="flex items-start gap-3 py-3 border-b border-stone-50 dark:border-zinc-800/60 last:border-0">
                    ...
                    <div>
                      <p className="text-xs text-stone-400 dark:text-zinc-500 mb-0.5">{field.label}</p>
                      <p className={`text-sm font-medium ${field.value === 'Not set' ? 'text-stone-400 dark:text-zinc-600 italic' : 'text-stone-900 dark:text-white'}`}>
                        {field.value}
                      </p>
                    </div>
                  </div>
          ...
          <div className="card p-6">
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-display text-xl font-bold text-stone-900 dark:text-white">Recent Orders</h3>
              <Link href="/orders" className="text-sm text-brand-500 hover:text-brand-600 font-medium">View all →</Link>
            </div>
            ...
            {ordersLoading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-stone-500 dark:text-zinc-400 text-sm">No orders yet</p>
                <Link href="/" className="text-sm text-brand-500 hover:text-brand-600 font-medium mt-2 inline-block">
                  Start shopping →
                </Link>
              </div>
            ) : (
              <div className="space-y-3">
                {orders.slice(0, 4).map(order => (
                  <Link key={order._id} href={`/orders/${order._id}`} className="flex items-center gap-4 p-3 rounded-xl hover:bg-stone-50 dark:hover:bg-zinc-800 transition-colors group">
                    {/* Thumbnails */}
                    <div className="flex -space-x-2">
                      {order.items.slice(0, 2).map((item, i) => (
                        <div key={i} className="relative w-12 h-12 rounded-xl overflow-hidden border-2 border-white dark:border-zinc-900 bg-stone-100 dark:bg-zinc-800">
                          <Image src={item.image} alt={item.name} fill className="object-cover" />
                        </div>
                      ))}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-mono text-stone-400 dark:text-zinc-500">#{order._id.slice(-8).toUpperCase()}</p>
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      <span className={`badge text-xs ${statusColors[order.status]} mt-1`}>
