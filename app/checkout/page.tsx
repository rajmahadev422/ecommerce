...
import { CreditCard, Lock, ChevronRight, Loader2, Check, MapPin } from 'lucide-react'
...
export default function CheckoutPage() {
  ...
  if (cart.items.length === 0 && step !== 'confirming') {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center page-enter">
        ...
        <p className="text-stone-500 dark:text-zinc-400 mb-8">Add some products before checking out</p>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 page-enter">
      {/* Header */}
      <div className="mb-8">
        <h1 className="font-display text-3xl font-bold text-stone-900 dark:text-white">Checkout</h1>
        {/* Steps indicator */}
          ...
          {['address', 'payment'].map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
                ...
                (step === 'payment' && s === 'address') || step === 'confirming' ? 'bg-green-500 text-white' :
                'bg-stone-100 dark:bg-zinc-800 text-stone-400 dark:text-zinc-500'
              }`}>
                {((step === 'payment' && s === 'address') || step === 'confirming') ? <Check className="w-4 h-4" /> : i + 1}
              </div>
              <span className={`text-sm font-medium capitalize ${step === s ? 'text-stone-900 dark:text-white' : 'text-stone-400 dark:text-zinc-500'}`}>{s}</span>
              {i === 0 && <ChevronRight className="w-4 h-4 text-stone-300 dark:text-zinc-700" />}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-3">
          {step === 'address' && (
            <form onSubmit={handleAddressSubmit} className="card p-6 space-y-5 animate-fade-in">
              <div className="flex items-center gap-3 mb-2">
                <MapPin className="w-5 h-5 text-brand-500" />
                <h2 className="font-display text-xl font-bold text-stone-900 dark:text-white">Shipping Address</h2>
              </div>
              <input type="text" placeholder="Full name" required value={address.name} onChange={e => setAddress({...address, name: e.target.value})} className="input-field" />
              <input type="text" placeholder="Street address" required value={address.street} onChange={e => setAddress({...address, street: e.target.value})} className="input-field" />
              ...
              <div className="grid grid-cols-2 gap-4">
                <input type="text" placeholder="ZIP Code" required value={address.zip} onChange={e => setAddress({...address, zip: e.target.value})} className="input-field" />
              ...
              {!user && (
                <div className="p-4 rounded-xl bg-brand-50 dark:bg-brand-950/30 border border-brand-200 dark:border-brand-800">
                  <p className="text-sm text-brand-700 dark:text-brand-300">
                    <button type="button" onClick={() => setAuthOpen(true)} className="font-semibold underline">Sign in</button>{' '}
          ...
          {(step === 'payment' || step === 'confirming') && (
            <form onSubmit={handlePayment} className="card p-6 space-y-5 animate-fade-in">
              <div className="flex items-center gap-3 mb-2">
                <Lock className="w-5 h-5 text-brand-500" />
                <h2 className="font-display text-xl font-bold text-stone-900 dark:text-white">Payment Details</h2>
              ...
              <div className="flex gap-3">
                <button type="button" onClick={() => setStep('address')} className="btn-secondary flex-1">
                  Back
                </button>
                <button
                  type="submit"
        ...
        <div className="lg:col-span-2">
          <div className="card p-6 sticky top-24">
            <h3 className="font-display text-lg font-bold text-stone-900 dark:text-white mb-4">Order Summary</h3>
              ...
              {cart.items.map(item => (
                <div key={item.productId} className="flex gap-3 items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-stone-500 dark:text-zinc-400">${item.price.toFixed(2)} each</p>
                  </div>
