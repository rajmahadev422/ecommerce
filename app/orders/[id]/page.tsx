...
import { ArrowLeft, Package, MapPin, CreditCard, Copy, Check, RefreshCw } from 'lucide-react'
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="card p-6 space-y-4">
              {[1,2,3,4,5,6].map(i => <div key={i} className="h-16 skeleton rounded-xl" />)}
            </div>
          </div>
  ...
  if (!order) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center page-enter">
        <h2 className="font-display text-2xl font-bold text-stone-900 dark:text-white">Order not found</h2>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 page-enter">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4 mb-8">
        <div>
          <h1 className="font-display text-3xl font-bold text-stone-900 dark:text-white">Order Details</h1>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-stone-400 dark:text-zinc-500 font-mono text-sm">#{order._id.slice(-8).toUpperCase()}</span>
            <button onClick={copyOrderId} className="p-1 text-stone-400 hover:text-brand-500 transition-colors">
        ...
        <button onClick={handleRefresh} className="flex items-center gap-2 btn-secondary text-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="card p-6">
            ...
            <OrderTimeline status={order.status as Parameters<typeof OrderTimeline>[0]['status']} statusHistory={order.statusHistory as Parameters<typeof OrderTimeline>[0]['statusHistory']} />

            {order.trackingNumber && (
              <div className="mt-4 p-3 rounded-xl bg-stone-50 dark:bg-zinc-800 flex items-center justify-between">
                <div>
                  <p className="text-xs text-stone-400 dark:text-zinc-500 font-medium">TRACKING NUMBER</p>
                  <p className="font-mono text-sm font-bold text-stone-900 dark:text-white mt-0.5">{order.trackingNumber}</p>
                </div>
              </div>
            )}
            ...
            {order.estimatedDelivery && order.status !== 'delivered' && order.status !== 'cancelled' && (
              <div className="mt-3 p-3 rounded-xl bg-brand-50 dark:bg-brand-950/20 border border-brand-100 dark:border-brand-900/30">
                <p className="text-sm text-brand-700 dark:text-brand-300">
                  📦 Estimated delivery:{' '}
                  <strong>{new Date(order.estimatedDelivery).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</strong>
                </p>
              </div>
            )}
          </div>
          ...
          <div className="card p-6">
            ...
            <div className="space-y-4">
              {order.items.map((item, i) => (
                <div key={i} className="flex gap-4 items-center py-3 border-b border-stone-50 dark:border-zinc-800 last:border-0 last:pb-0">
                  <div className="relative w-16 h-16 rounded-xl overflow-hidden bg-stone-100 dark:bg-zinc-800 flex-shrink-0">
                    <Image src={item.image} alt={item.name} fill className="object-cover" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <Link href={`/products/${item.productId}`} className="font-medium text-stone-900 dark:text-white hover:text-brand-500 transition-colors line-clamp-1">
                      {item.name}
                    </Link>
        ...
        <div className="space-y-5">
          ...
          <div className="card p-5">
            <h3 className="font-display text-lg font-bold text-stone-900 dark:text-white mb-4">Summary</h3>
            ...
            <div className="mt-4 pt-3 border-t border-stone-100 dark:border-zinc-800 flex items-center gap-2">
              ...
              <div>
                <p className="text-xs text-stone-400 dark:text-zinc-500">Payment</p>
                <p className={`text-sm font-semibold capitalize ${paymentColors[order.paymentStatus] || 'text-stone-600'}`}>
                  {order.paymentStatus}
                </p>
              </div>
            </div>
          ...
          <div className="card p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-display text-lg font-bold text-stone-900 dark:text-white">Shipping To</h3>
            </div>
            <div className="text-sm text-stone-600 dark:text-zinc-400 space-y-0.5">
              <p className="font-semibold text-stone-900 dark:text-white">{order.shippingAddress.name}</p>
